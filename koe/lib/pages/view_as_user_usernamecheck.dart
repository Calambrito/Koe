...
class ViewAsUserUsernameCheck extends StatefulWidget {
  ...
  @override
  State<ViewAsUserUsernameCheck> createState() =>
      _ViewAsUserUsernameCheckState();
}
    ...
    try {
      ...
      if (!exists) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('User "$input" not found')));
        return;
      }
      ...
      Navigator.push(
        context,
        MaterialPageRoute(builder: (_) => ViewAsUserHomepage(userName: input)),
      );
    } finally {
